...
function PrivateShare({ type, data, handleDelete, children, handleOnLike }) {
  ...
  const url =
    selectedOption === "Public"
      ? `${SHARE}/${type}/${data?._id}`
      : `${SHARE}/${type}/${data?._id}/${data?.tokenId}`;
  ...
  const handleChangeVisibilty = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API}/api/${type}/change/${data?._id}/${selectedOption}`
      );
              ...
              <div className="p-4 md:p-5 space-y-4">
                <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                  {selectedOption === "Public"
                    ...
                    : "If you make this private then it will be viewed only by share url"}
                </p>
                <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              ...
              <div className="p-4 md:p-5 space-y-4 items-center flex flex-col">
                <div className="w-full max-w-sm">
                  <div className="flex items-center">
                    <span
                      className="flex-shrink-0 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-s-lg dark:bg-gray-600 dark:text-white dark:border-gray-600"
                      style={{ zIndex: 1 }}
                    >
                      URL
                    </span>
                    ...
                    >
                      <span
                        id="default-icon"
                        className={
                          !isCopied ? "inline-flex items-center" : "hidden"
                        }
                      >
                        <svg
                          className="w-4 h-4"
  ...
  <li>
    <Link
      ...
      className="relative bg-white overflow-hidden rounded-full w-12 h-12 flex justify-center items-center flex-col shadow-xl cursor-pointer transition-all duration-200 ease-out hover:scale-105 hover:bg-opacity-70"
    >
      <img
  ...
  <li>
    <Link
      to={`https://www.instagram.com/share?url=${encodeURIComponent(url)}`}
      target="_blank"
      rel="noopener noreferrer"
      className="relative bg-white overflow-hidden rounded-full w-12 h-12 flex justify-center items-center flex-col shadow-xl cursor-pointer transition-all duration-200 ease-out hover:scale-105 hover:bg-opacity-70"
    >
  ...
  <li>
    <Link
      to={`https://wa.me/?text=${encodeURIComponent(url)}`}
      target="_blank"
      rel="noopener noreferrer"
      className="relative bg-white overflow-hidden rounded-full w-12 h-12 flex justify-center items-center flex-col shadow-xl cursor-pointer transition-all duration-200 ease-out hover:scale-105 hover:bg-opacity-70"
    >
