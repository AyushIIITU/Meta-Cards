...
// import wishDisplay from "../Link/CakeDisplay";
...
function PrivateWish() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`${API}/api/wish/${id}`);
  ...
  const handleOnLike = async (id) => {
    try {
      ...
      if (liked) {
        ...
        const response = await axios.post(`${API}/api/wish/unlike`, {
          id: id,
          user: userId,
        });
      } else {
        localStorage.setItem(`isLiked-${id}`, true);
        const response = await axios.post(`${API}/api/wish/like`, {
          id: id,
          user: userId,
        });
      }
